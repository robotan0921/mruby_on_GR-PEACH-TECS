...
class UDP < TECS::TsMrubyUDP
	...
	public
		...
		def bind(addr, port)
			addr = addr.split(".")
			@myaddr = (addr[0].to_i << 24) | (addr[1].to_i << 16) | (addr[2].to_i << 8) | (addr[3].to_i)
			@myport = port
		end
		...
		def snd msg
			...
			puts ">>snd<< --> msg_buf 2"
			send(msg_buf, len, @dstaddr, @dstport, 10000000)
			puts ">>snd<< --> send"
		end
		def recv
...
begin
	puts "[mruby]:udp_sample [TaskID:#{RTOS.id}]"

	cep = UDP.new()
	# cep.connect('192.168.1.56', 8931)
	cep.connect(192, 168, 1, 56, 8931)
	...
	puts "UDP sample application is ended!! [TaskID:#{RTOS.id}]"
