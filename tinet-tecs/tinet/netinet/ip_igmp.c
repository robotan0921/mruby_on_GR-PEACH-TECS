...
*
*  @(#) $Id: ip_igmp.c 936 2016-05-02 11:18:27Z coas-nagasima $
*/
...
static uint16_t igmp_get_timer(T_IGMP_ENTRY *entry);
...
static bool_t igmp_send_report_v2(T_IN4_ADDR dst)
{
	...
	/* 4 オクテット境界までパディングで埋める。*/
	if (align > len)
		memset((uint8_t*)GET_IGMP_SDU(output, IF_IP4_IGMP_HDR_OFFSET) + IP4_HDR_SIZE + len,
			0, (size_t)(align - len));

	/* チェックサムを計算する。*/
	igmph->sum = 0;
	igmph->sum = in_cksum(igmph, (uint_t)(IF_IP4_IGMP_HDR_SIZE + align));
...
uint16_t igmp_get_timer(T_IGMP_ENTRY *entry)
{
	uint16_t time = entry->resptime;

	if (time == 0)
		time = 10;

	return net_rand() / (0x7fffffff / time);
}
