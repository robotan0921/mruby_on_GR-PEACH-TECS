 ...
 * 
 *  @(#) $Id: dbg_cons.c 936 2016-05-02 11:18:27Z coas-nagasima $
 */
...
dbg_cons_network_status (ID portid, char *line)
{
	...
	case 'r':		/* ルーティング情報 */
		routing_table_status(portid, line + 1);
		break;
...
dbg_cons_ping_comd (ID portid, char *line)
{
	...
	if ('0' <= *line && *line <= '9') {
		if (*line == '6')
			proto = API_PROTO_IPV6;
		if (*line == '4')
			proto = API_PROTO_IPV4;
		line ++;
		}

#endif	/* of #if defined(SUPPORT_INET6) && defined(SUPPORT_INET4) */

	if ((line = lookup_ipaddr(&addr, line)) == NULL) {
