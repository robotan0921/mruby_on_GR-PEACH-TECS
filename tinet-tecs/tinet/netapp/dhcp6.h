 ...
 * 
 *  @(#) $Id: dhcp6.h 968 2016-06-19 14:49:07Z coas-nagasima $
 */
...
typedef struct dhcp6_msg_hdr {
	...
	} __attribute__((packed))T_DHCP6_MSG_HDR;

typedef struct dhcp6_msg {
	T_DHCP6_MSG_HDR	hdr;
	uint8_t		options[DHCP6_OPTION_LEN];	/* options	*/
	} __attribute__((packed))T_DHCP6_MSG;
...
typedef struct dhcp6_option {
	...
	} __attribute__((packed))T_DHCP6_OPTION;
...
typedef struct dhcp6_opt_clientid {
	uint16_t	code;		/* option code		*/
	uint16_t	len;		/* option length	*/
	} __attribute__((packed))T_DHCP6_OPT_CLIENTID;
...
typedef struct dhcp6_opt_serverid {
	uint16_t	code;		/* option code		*/
	uint16_t	len;		/* option length	*/
	} __attribute__((packed))T_DHCP6_OPT_SERVERID;
...
typedef struct dhcp6_opt_ia_na_pd {
	...
	} __attribute__((packed))T_DHCP6_OPT_IA_NA_PD;
...
typedef struct dhcp6_opt_ia_ta {
	...
	uint16_t	len;		/* option length	*/
	uint32_t	iaid;		/* IA ID		*/
	} __attribute__((packed))T_DHCP6_OPT_IA_TA;
...
typedef struct dhcp6_opt_iaaddr {
	...
	} __attribute__((packed))T_DHCP6_OPT_IAADDR;
...
typedef struct dhcp6_opt_oro {
	uint16_t	code;		/* option code		*/
	uint16_t	len;		/* option length	*/
	} __attribute__((packed))T_DHCP6_OPT_ORO;
...
typedef struct dhcp6_opt_preference {
	...
	} __attribute__((packed))T_DHCP6_OPT_PREFERENCE;
...
typedef struct dhcp6_opt_unicast {
	...
	} __attribute__((packed))T_DHCP6_OPT_UNICAST;
...
typedef struct dhcp6_opt_status_code {
	...
	} __attribute__((packed))T_DHCP6_OPT_STATUS_CODE;
...
typedef struct dhcp6_opt_name_servers {
	...
	} __attribute__((packed))T_DHCP6_OPT_NAME_SERVERS;
...
typedef struct dhcp6_opt_domain_search {
	...
	} __attribute__((packed))T_DHCP6_OPT_DOMAIN_SEARCH;
...
typedef struct dhcp6_duid_llt_hdr {
	...
	uint32_t	time;		/* time			*/
	} __attribute__((packed))T_DHCP6_DUID_LLT_HDR;
...
typedef struct dhcp6_duid_ll_hdr {
	uint16_t	code;		/* DUID code		*/
	uint16_t	type;		/* hardware type	*/
	} __attribute__((packed))T_DHCP6_DUID_LL_HDR;
