# -*- coding: iso-2022-jp -*-
class RepeatPartPlugin < CellPlugin

  RepeatPartPluginArgProc = {
    "count" => Proc.new { |obj,rhs| obj.set_count rhs }
  }

  def initialize( cell, option )
    super
    print "RepeatPartPlugin: #{@cell.get_name}\n"
    # cell.show_tree 0
    @plugin_arg_check_proc_tab = RepeatPartPluginArgProc
    parse_plugin_arg
    ...
    @cell.get_join_list.get_items.each{ |j|
      ...
      if j.get_rhs.instance_of?(Array) then
        ...
        #定数の場合はその値を繰り返し代入してあげる
        if j.get_rhs[0].get_elements[0] == :INTEGER_CONSTANT then
          temp_token = j.get_rhs[0].get_elements[1]
          while count < @count
            j.get_rhs[count] = Expression.new([:INTEGER_CONSTANT,temp_token])
            count += 1
          end
          next
        end
      ...
      #こちらのプラグインの場合は接続されるセルの末尾が数字じゃなければ、リピートしない
