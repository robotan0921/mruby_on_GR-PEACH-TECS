 ...
 * 
 *  @(#) $Id: netapp_subr.c 936 2016-05-02 11:18:27Z coas-nagasima $
 */
...
static const T_IN6_ADDR in6_addr_unspecified =
...
char *
resolv_options (uint_t *flags, char *line)
{
#if defined(SUPPORT_INET6)
	T_IN6_ADDR	in6_addr;
#endif
	...
	*flags |= DNS_LUP_FLAGS_PROTO_IPV6 | DNS_LUP_FLAGS_QTYPE_AAAA
	       |  DNS_LUP_FLAGS_PROTO_IPV4 | DNS_LUP_FLAGS_QTYPE_A;
	if (*line == '-') {
		line ++;
		if (*line == '6') {
			line ++;
...
char *
lookup_ipaddr (T_IN_ADDR *addr, char *line)
{
	...
	line = skip_blanks(resolv_options(&flags, line));
