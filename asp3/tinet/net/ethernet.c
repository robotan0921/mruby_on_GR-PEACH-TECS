 ...
 * 
 *  @(#) $Id: ethernet.c 965 2016-06-18 02:49:26Z coas-nagasima $
 */
...
{
  if (!(ether->_flags & IF_FLAG_LINK_UP)) {
    ether->_flags |= IF_FLAG_LINK_UP;

    if (ether->_flags & IF_FLAG_UP) {
#if LWIP_ARP
      /* For Ethernet network interfaces, we would like to send a "gratuitous ARP" */ 
      if (ether->_flags & IF_FLAG_ETHARP) {
      ...
      if (ether->_flags & IF_FLAG_IGMP) {
        igmp_report_groups(ether);
      }
#endif /* LWIP_IGMP */
    }
